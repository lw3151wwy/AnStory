...
public class AnimatedGifEncoder1 {
	...
	public boolean addFrame(Bitmap paramBitmap) {
		Log.v("123","beginAddFrame" + System.currentTimeMillis());
		boolean ok = true;
		...
		try {
			...
			Log.v("123","beginImagePixels" + System.currentTimeMillis());
			getImagePixels();
			Log.v("123","beginanalyzePixels" + System.currentTimeMillis());
			analyzePixels();
			if (firstFrame) {
				Log.v("123","beginFirstFrame" + System.currentTimeMillis());
				writeLSD();
				writePalette();
				if (repeat >= 0)
					writeNetscapeExt();
			}
			Log.v("123","beginWriteCtrl" + System.currentTimeMillis());
			writeGraphicCtrlExt();
			Log.v("123","beginWriteImageDesc" + System.currentTimeMillis());
			writeImageDesc();

			if (!firstFrame)
				writePalette();
			Log.v("123","beginWritePixels" + System.currentTimeMillis());
			writePixels();
		...
		Log.v("123","AddFrameover" + System.currentTimeMillis());
		return ok;
	}
